...
inline float normalizeToMinus1To1(float x, float xMin, float xMax) {
	float b;
	float a = (x - xMin) / (xMax - xMin); // normalize to [0, 1]
	b = a * 2.0f - 1.0f; // scale to [-1, 1]
	return clamp(b, -1.0f, 1.0f);
}

inline float denormalizeFromMinus1To1(float xNorm, float xMin, float xMax) {
    float a = (xNorm + 1.0f) / 2.0f; // map [-1, 1] to [0, 1]
...
public:
	...
	FuzzyRule(  const FuzzyCondition& inputMF1,
				const FuzzyCondition& inputMF2,
				PtrFuzzyOperator op,
...
private:
	const FuzzyCondition& m_inputA, m_inputB;
