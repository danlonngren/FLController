...
TEST_F(FLControllerTests, FLControllerEvaluatePIDTest) {
	...
	std::array<TestData, 11> testInput = {
		...
		TestData(60.0f,   7.3627701f),
		TestData(50.0f,   5.4783478f),
		TestData(40.0f,   3.5930862f),
		TestData(30.0f,   1.7069321f),
		TestData(20.0f,   -0.18021393f),
		TestData(10.0f,   -2.068367f),
		TestData(0.0f,    -3.9575195f),
	};
	...
	for (const auto& data : testInput) {
		...
		m_pData.set(normalizeToMinus1To1(data.input, -100.0f, 100.0f));
		m_iData.set(normalizeToMinus1To1(i, -100.0f, 100.0f));
		m_dData.set(normalizeToMinus1To1(d, -100.0f, 100.0f));
		float result = m_controller->evaluate();
		EXPECT_FLOAT_EQ(denormalizeFromMinus1To1(result, -100.0f, 100.0f), data.expected);
	}
}